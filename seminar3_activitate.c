...
int main()
{
	...
	//memset(catalog, 0, sizeof(catalog)); fara asta
	...
	if (pFile)
	{
		...
		*/
		...
		while (fgets(buffer, sizeof(buffer), pFile))
		{
			...
			//catalog[index++] = stud;
            insertStudent(&catalog,stud,&size); //transmitem prin adresa adica ii transmitem referinta pentru a se modifica
			printStudent(stud); // trb sa facem un for loop pe catalog cu print student ca sa ii afiseze ordonat
		}
	}
